...
values. A position in memory is called an address (for example, the
...
pointer; it starts at 0. After an instruction finishes, the instruction
pointer increases by the number of values in the instruction; until you
add more instructions to the computer, this is always 4 (1 opcode +
3 parameters) for the add and multiply instructions. (The halt
...
def process_opcode(m):
    position = 0

    for position in range(0, len(m), 4):
        opcode = m[position]
        if opcode == 1:
            # adds
            a = m[m[position + 1]]
...
def load_intcode(FILEPATH):
    ...
    with open(FILEPATH) as f:
        ...
        while line:
            intcode_map += [int(n) for n in line.strip().split(',')]
            line = f.readline()
...
def calculate_target(FILEPATH, target):
    for noun in range(100):
        for verb in range(100):
            ...
            if m[0] == target:
                return noun, verb, m
    # Noun and verb are in range 0-99
    # return -1 when could not find values to reach the target
    return -1, -1, m
